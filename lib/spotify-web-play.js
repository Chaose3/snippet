...
function buildPlayRequest(trackUri, positionMs, contextSource, deviceId = null) {
  const usePlaylistContext = Boolean(contextSource?.contextUri && contextSource?.offsetUri);
  return {
    trackUri,
    positionMs,
    deviceId,
    contextUri: usePlaylistContext ? contextSource.contextUri : null,
    offsetUri: usePlaylistContext ? contextSource.offsetUri : null,
    offsetPosition: usePlaylistContext ? contextSource.offsetPosition : undefined,
  };
}
...
export async function playViaSpotifyWebApi(accessToken, trackUri, positionMs = 0, contextSource = null) {
  ...
  if (targetDevice) {
    ...
    if (result.ok && !result.verified) {
      return { ok: false, status: 404, verified: false };
    }
  ...
  if (fallback.ok && fallback.verified) return fallback;
