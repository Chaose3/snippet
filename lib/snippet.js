/**
 * Hardcoded MVP snippet. Later: load from API / user saves / slider UI.
 */
export const DEFAULT_SNIPPET = {
  id: "mvp-1",
  label: "Demo (Cut To The Feeling)",
  trackUri: "spotify:track:11dFghVXANMlKmJXsNCbNl",
  positionMs: 60000,
};

/**
 * Clone snippet with a new start time (ms). Useful for sliders / multiple snippets.
 */
export function snippetAtPositionMs(snippet, positionMs) {
  return { ...snippet, positionMs: Math.max(0, Math.floor(positionMs)) };
}

function isTokenExpiredResponse(res) {
  return res?.status === 401;
}

/** Spotify only allows /playlists/{id}/items for playlists you own (not followed/editorial). */
export const PLAYLIST_ITEMS_FORBIDDEN_MESSAGE =
  "Spotify doesn't allow third-party apps to load tracks for this playlist. That usually means it's a followed playlist, a Spotify playlist (Discover Weekly, etc.), or one you don't own. Try a playlist you created in Spotify.";

export const RECENTLY_PLAYED_SCOPE_MESSAGE =
  "Recently played needs the user-read-recently-played permission. Log out, connect again, and approve all permissions.";

async function safeSpotifyFetch(url, options = {}, label = "spotifyFetch") {
  try {
    return await fetch(url, options);
  } catch (error) {
    console.warn(`[${label}] network failure`, error);
    return null;
  }
}

/**
 * Fetch available Spotify devices for the user.
 */
export async function getDevices(accessToken) {
  const res = await safeSpotifyFetch("https://api.spotify.com/v1/me/player/devices", {
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "getDevices");
  if (!res) return [];
  if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
  if (!res.ok) return [];
  const data = await res.json();
  return (data.devices || [])
    .filter((d) => !d.is_restricted)
    .map((d) => ({
      id: d.id,
      name: d.name,
      type: d.type,
      isActive: d.is_active,
    }));
}

/**
 * Transfer playback to a specific Spotify device.
 */
export async function transferPlayback(accessToken, deviceId, shouldPlay = false) {
  return safeSpotifyFetch("https://api.spotify.com/v1/me/player", {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      device_ids: [deviceId],
      play: shouldPlay,
    }),
  });
}

/**
 * PUT /v1/me/player/play — playback only via Spotify; no audio storage.
 * Pass deviceId to start playback on a specific device when nothing is active.
 */
export async function playSnippet(accessToken, snippet) {
  const body = snippet.contextUri
    ? {
        context_uri: snippet.contextUri,
        offset: snippet.offsetUri
          ? { uri: snippet.offsetUri }
          : typeof snippet.offsetPosition === "number"
            ? { position: snippet.offsetPosition }
            : undefined,
        position_ms: snippet.positionMs,
      }
    : {
        uris: [snippet.trackUri],
        position_ms: snippet.positionMs,
      };

  const url = snippet.deviceId
    ? `https://api.spotify.com/v1/me/player/play?device_id=${snippet.deviceId}`
    : "https://api.spotify.com/v1/me/player/play";

  console.log("[playSnippet] request", {
    trackUri: snippet.trackUri,
    contextUri: snippet.contextUri ?? null,
    offsetUri: snippet.offsetUri ?? null,
    offsetPosition: snippet.offsetPosition ?? null,
    position_ms: snippet.positionMs,
    deviceId: snippet.deviceId ?? null,
  });

  const res = await safeSpotifyFetch(url, {
    method: "PUT",
    headers: {
      Authorization: `Bearer ${accessToken}`,
      "Content-Type": "application/json",
    },
    body: JSON.stringify(body),
  }, "playSnippet");

  if (!res) return { ok: false, status: 0, text: async () => "" };

  console.log("[playSnippet] response status", res.status);

  if (!res.ok) {
    const text = await res.text();
    console.warn("[playSnippet] body", text);
  }

  return res;
}

/**
 * Get full player state — track, position, duration, album art, play/pause.
 */
export async function getPlayerState(accessToken) {
  const res = await safeSpotifyFetch("https://api.spotify.com/v1/me/player", {
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "getPlayerState");

  if (!res) return null;
  if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
  if (res.status === 204 || !res.ok) return null;

  const data = await res.json();
  if (!data || !data.item) return null;

  return {
    id: data.item.id,
    name: data.item.name,
    uri: data.item.uri,
    artists: data.item.artists.map((a) => a.name).join(", "),
    albumArt: data.item.album.images[0]?.url ?? null,
    durationMs: data.item.duration_ms,
    positionMs: data.progress_ms,
    isPlaying: data.is_playing,
    shuffle: data.shuffle_state,
    repeatMode: data.repeat_state ?? "off",
    volumePercent: data.device?.volume_percent ?? null,
    deviceName: data.device?.name ?? null,
    deviceType: data.device?.type ?? null,
    contextUri: data.context?.uri ?? null,
    contextType: data.context?.type ?? null,
  };
}

/**
 * Fetch a single track by Spotify id (for cold-open /player/[id]).
 */
export async function getTrackById(accessToken, trackId) {
  const res = await safeSpotifyFetch(
    `https://api.spotify.com/v1/tracks/${encodeURIComponent(trackId)}`,
    {
      headers: { Authorization: `Bearer ${accessToken}` },
    },
    "getTrackById"
  );
  if (!res) return null;
  if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
  if (res.status === 404) return null;
  if (!res.ok) return null;
  const data = await res.json();
  if (!data?.id) return null;
  return {
    id: data.id,
    name: data.name,
    uri: data.uri,
    artists: (data.artists || []).map((a) => a.name).join(", "),
    albumArt: data.album?.images?.[0]?.url ?? null,
    durationMs: data.duration_ms,
  };
}

/**
 * Seek to a position in the current track (ms).
 */
export async function seekToPosition(accessToken, positionMs) {
  return safeSpotifyFetch(`https://api.spotify.com/v1/me/player/seek?position_ms=${positionMs}`, {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "seekToPosition");
}

/**
 * Pause playback.
 */
export async function pausePlayback(accessToken) {
  return safeSpotifyFetch("https://api.spotify.com/v1/me/player/pause", {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "pausePlayback");
}

/**
 * Resume playback.
 */
export async function resumePlayback(accessToken) {
  return safeSpotifyFetch("https://api.spotify.com/v1/me/player/play", {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "resumePlayback");
}

/**
 * Set volume (0–100).
 */
export async function setVolume(accessToken, volumePercent) {
  return safeSpotifyFetch(`https://api.spotify.com/v1/me/player/volume?volume_percent=${volumePercent}`, {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "setVolume");
}

/**
 * Toggle shuffle on or off.
 */
export async function setShuffle(accessToken, state) {
  return safeSpotifyFetch(`https://api.spotify.com/v1/me/player/shuffle?state=${state}`, {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "setShuffle");
}

/**
 * Cycle / set repeat mode.
 */
export async function setRepeatMode(accessToken, state, deviceId) {
  const url = deviceId
    ? `https://api.spotify.com/v1/me/player/repeat?state=${state}&device_id=${deviceId}`
    : `https://api.spotify.com/v1/me/player/repeat?state=${state}`;
  return safeSpotifyFetch(url, {
    method: "PUT",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "setRepeatMode");
}

/**
 * Skip to the next track in queue.
 */
export async function skipToNext(accessToken, deviceId) {
  const url = deviceId
    ? `https://api.spotify.com/v1/me/player/next?device_id=${deviceId}`
    : "https://api.spotify.com/v1/me/player/next";
  return safeSpotifyFetch(url, {
    method: "POST",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "skipToNext");
}

/**
 * Skip to the previous track in queue.
 */
export async function skipToPrevious(accessToken, deviceId) {
  const url = deviceId
    ? `https://api.spotify.com/v1/me/player/previous?device_id=${deviceId}`
    : "https://api.spotify.com/v1/me/player/previous";
  return safeSpotifyFetch(url, {
    method: "POST",
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "skipToPrevious");
}

/** Normalize a track from queue / player payloads (track object or wrapped). */
export function mapSpotifyTrackItem(item) {
  if (!item) return null;
  if (item.type === "episode") return null;
  const track =
    item.type === "track" || (typeof item.uri === "string" && item.uri.startsWith("spotify:track:"))
      ? item
      : item.track ?? null;
  if (!track?.id) return null;
  return {
    id: track.id,
    name: track.name,
    uri: track.uri,
    artists: (track.artists || []).map((a) => a.name).join(", "),
    durationMs: track.duration_ms ?? 0,
    albumArt: track.album?.images?.[0]?.url ?? null,
  };
}

/**
 * Get the user's current queue.
 */
export async function getQueue(accessToken) {
  const res = await safeSpotifyFetch("https://api.spotify.com/v1/me/player/queue", {
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "getQueue");
  if (!res) return [];
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    console.warn("[getQueue] failed", res.status, body);
    if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
    return [];
  }
  const data = await res.json();
  const nowPlayingId =
    data.currently_playing?.type === "track" ? data.currently_playing.id : data.currently_playing?.id ?? null;
  const seen = new Set();
  const tracks = [];
  for (const item of data.queue || []) {
    const mapped = mapSpotifyTrackItem(item);
    if (!mapped || mapped.id === nowPlayingId || seen.has(mapped.id)) continue;
    seen.add(mapped.id);
    tracks.push(mapped);
  }
  return tracks;
}

/**
 * Fetch the user's liked songs (up to 50).
 */
export async function getLikedTracks(accessToken) {
  const tracks = [];
  let url = "https://api.spotify.com/v1/me/tracks?limit=50";

  while (url) {
    const res = await safeSpotifyFetch(url, {
      headers: { Authorization: `Bearer ${accessToken}` },
    }, "getLikedTracks");
    if (!res) break;
    if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
    if (!res.ok) break;
    const data = await res.json();
    const page = (data.items || [])
      .filter((item) => item.track?.id)
      .map((item) => ({
        id: item.track.id,
        name: item.track.name,
        uri: item.track.uri,
        artists: item.track.artists.map((a) => a.name).join(", "),
        durationMs: item.track.duration_ms,
        albumArt: item.track.album.images?.[0]?.url ?? null,
      }));
    tracks.push(...page);
    url = data.next ?? null;
  }

  return tracks;
}

/**
 * Current Spotify user (for playlist ownership checks).
 */
export async function getCurrentUserProfile(accessToken) {
  const res = await safeSpotifyFetch("https://api.spotify.com/v1/me", {
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "getCurrentUserProfile");
  if (!res) return null;
  if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    console.warn("[getCurrentUserProfile] failed", res.status, body);
    return null;
  }
  const data = await res.json();
  if (!data?.id) return null;
  return {
    id: data.id,
    displayName: data.display_name ?? "",
    country: data.country ?? null,
    product: data.product ?? null,
  };
}

/**
 * Fetch the user's recently played tracks in Spotify's returned order.
 * @returns {Promise<{ tracks: Array, status: number, error?: string }>}
 */
export async function getRecentlyPlayed(accessToken) {
  const res = await safeSpotifyFetch("https://api.spotify.com/v1/me/player/recently-played?limit=20", {
    headers: { Authorization: `Bearer ${accessToken}` },
  }, "getRecentlyPlayed");
  if (!res) {
    return { tracks: [], status: 0, error: "Network error loading recently played." };
  }
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    console.warn("[getRecentlyPlayed] failed", res.status, body);
    if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
    if (res.status === 401 || res.status === 403) {
      return {
        tracks: [],
        status: res.status,
        error: RECENTLY_PLAYED_SCOPE_MESSAGE,
      };
    }
    return {
      tracks: [],
      status: res.status,
      error: `Couldn't load recently played (HTTP ${res.status}).`,
    };
  }
  const data = await res.json();
  const seen = new Set();
  const tracks = (data.items || [])
    .map((entry) => entry?.track ?? null)
    .filter((track) => track?.id && !seen.has(track.id) && seen.add(track.id))
    .map((track) => ({
      id: track.id,
      name: track.name,
      uri: track.uri,
      artists: track.artists.map((a) => a.name).join(", "),
      durationMs: track.duration_ms,
      albumArt: track.album?.images?.[0]?.url ?? null,
    }));
  return { tracks, status: res.status };
}

/**
 * Fetch all of the user's playlists, paginating through every page.
 */
export async function getUserPlaylists(accessToken) {
  const playlists = [];
  let url = "https://api.spotify.com/v1/me/playlists";

  while (url) {
    const res = await safeSpotifyFetch(url, {
      headers: { Authorization: `Bearer ${accessToken}` },
    }, "getUserPlaylists");
    if (!res) break;
    if (!res.ok) {
      const body = await res.text().catch(() => "");
      console.warn("[getUserPlaylists] failed", res.status, body);
      if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
      break;
    }
    const data = await res.json();
    (data.items || [])
      .filter((p) => p && p.id)
      .forEach((p) => {
        playlists.push({
          id: p.id,
          name: p.name,
          uri: p.uri ?? `spotify:playlist:${p.id}`,
          order: playlists.length,
          trackCount: p.tracks?.total ?? 0,
          coverArt: p.images?.[0]?.url ?? null,
          ownerId: p.owner?.id ?? null,
          ownerName: p.owner?.display_name ?? null,
        });
      });
    url = data.next ?? null;
  }

  return playlists;
}

/**
 * Fetch all tracks for a playlist, paginating through every page.
 */
export async function getPlaylistTracks(accessToken, playlistId, { market = null } = {}) {
  const tracks = [];
  const marketQs = market ? `&market=${encodeURIComponent(market)}` : "";
  let url = `https://api.spotify.com/v1/playlists/${playlistId}/items?limit=100&additional_types=track${marketQs}`;

  while (url) {
    const res = await safeSpotifyFetch(url, {
      headers: { Authorization: `Bearer ${accessToken}` },
    }, "getPlaylistTracks");
    if (!res) break;
    if (!res.ok) {
      const body = await res.text().catch(() => "");
      console.warn("[getPlaylistTracks] failed", playlistId, res.status, body);
      if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
      if (res.status === 403) {
        return { tracks: [], forbidden: true, message: PLAYLIST_ITEMS_FORBIDDEN_MESSAGE };
      }
      break;
    }
    const data = await res.json();
    (data.items || [])
      .map((entry) => entry?.item ?? entry?.track ?? null)
      .filter((item) => item?.type === "track" && item?.id)
      .forEach((item) => {
        tracks.push({
          id: item.id,
          name: item.name,
          uri: item.uri,
          contextUri: `spotify:playlist:${playlistId}`,
          offsetPosition: tracks.length,
          offsetUri: item.uri,
          artists: item.artists.map((a) => a.name).join(", "),
          durationMs: item.duration_ms,
          albumArt: item.album?.images?.[0]?.url ?? null,
        });
      });
    url = data.next ?? null;
  }

  return { tracks, forbidden: false, message: null };
}

/**
 * Search all of Spotify for tracks matching a query.
 */
export async function searchTracks(accessToken, query) {
  if (!query) return [];
  const params = new URLSearchParams({ q: query, type: "track" });
  const res = await safeSpotifyFetch(
    `https://api.spotify.com/v1/search?${params}`,
    { headers: { Authorization: `Bearer ${accessToken}` } },
    "searchTracks"
  );
  if (!res) return [];
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    console.warn("[searchTracks] failed", res.status, body);
    if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
    return [];
  }
  const data = await res.json();
  return (data.tracks?.items || [])
    .filter(Boolean)
    .map((t) => ({
      id: t.id,
      name: t.name,
      uri: t.uri,
      artists: t.artists.map((a) => a.name).join(", "),
      durationMs: t.duration_ms,
      albumArt: t.album.images?.[0]?.url ?? null,
    }));
}

/**
 * Get the currently playing track from Spotify.
 */
export async function getCurrentlyPlaying(accessToken) {
  const res = await safeSpotifyFetch("https://api.spotify.com/v1/me/player/currently-playing", {
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  }, "getCurrentlyPlaying");

  if (!res) return null;
  if (isTokenExpiredResponse(res)) throw new Error("TOKEN_EXPIRED");
  if (res.status === 204) return null;
  if (!res.ok) {
    console.warn("[getCurrentlyPlaying] failed", res.status);
    return null;
  }

  const data = await res.json();
  if (!data.item) {
    console.warn("[getCurrentlyPlaying] no item playing");
    return null;
  }

  return {
    id: data.item.id,
    name: data.item.name,
    uri: data.item.uri,
    artists: data.item.artists.map((a) => a.name).join(", "),
    albumArt: data.item.album?.images?.[0]?.url ?? null,
    durationMs: data.item.duration_ms,
    progressMs: data.progress_ms ?? 0,
    isPlaying: data.is_playing ?? false,
  };
}
